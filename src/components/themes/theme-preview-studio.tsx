...
type ThemePreviewStudioProps = {
  ...
  enabledThemeKeys: ThemeKey[];
  locale: AppLocale;
  themeChangedAt?: Date | string | null;
  cooldownDays: number;
};

export function ThemePreviewStudio({
  tenantSlug,
  currentThemeKey = themes[0].key,
  accessibleThemeKeys,
  enabledThemeKeys,
  locale,
  themeChangedAt,
  cooldownDays,
}: ThemePreviewStudioProps) {
  const safeCurrentTheme =
    themes.find((theme) => theme.key === currentThemeKey) ?? themes[0];
  const visibleThemes = themes.filter(
    (theme) =>
      enabledThemeKeys.includes(theme.key) ||
      theme.key === safeCurrentTheme.key,
  );
  ...
  const nextSwitchAt =
    lastChangedAt && cooldownDays > 0
      ? addDays(lastChangedAt, cooldownDays)
      : null;
  const switchingBlocked = Boolean(
    nextSwitchAt && nextSwitchAt.getTime() > Date.now(),
  );
  const cooldownMessage =
    switchingBlocked && nextSwitchAt
      ? `You can apply another theme on ${formatDate(nextSwitchAt)}.`
      : "";
  const switchNotice =
    cooldownDays > 0
  ...
  const confirmTheme = confirmThemeKey
    ? (visibleThemes.find((theme) => theme.key === confirmThemeKey) ?? null)
    : null;
    ...
    <article
      className={cn(
        ...
        applied
          ...
          : "border-slate-200 hover:border-slate-400 hover:shadow-[0_16px_34px_rgba(15,23,42,0.1)]",
      )}
    >
      <div className="relative aspect-[16/10] overflow-hidden bg-slate-100">
        ...
        <div className="absolute inset-x-0 top-0 flex items-start justify-between gap-2 p-3">
          {applied ? (
            <span className="rounded-full bg-teal-50 px-3 py-1 text-xs font-semibold text-teal-800 shadow-sm">
              Current
            </span>
          ) : (
            <ThemeBadge theme={theme} locked={locked} />
          )}
        </div>
      </div>
      ...
      <div className="flex flex-1 flex-col p-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h2 className="font-display text-2xl font-black tracking-[-0.04em] text-slate-950">
              {theme.name}
            </h2>
      ...
      <div className="flex items-center justify-between gap-2 border-t border-slate-200 bg-slate-50 p-3">
        ...
        <Button
          ...
          size="sm"
          onClick={onApply}
          disabled={applied || pending || locked || switchBlocked}
        >
          {locked ? (
...
function getLockedThemeLabel(theme: ThemeConfig) {
  return getThemeRequiredAccess(theme) === "pro"
    ? "Requires Pro"
    : "Requires paid plan";
}
