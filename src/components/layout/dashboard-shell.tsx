...
export function DashboardShell({
  ...
  function renderNavItems({
    ...
    return nav.map(({ label, href, icon: Icon }) => {
      const isActive =
        href === activeRootHref ? pathname === href : pathname.startsWith(href);
      const link = (
        <Link
          href={href as Route}
          onClick={onNavigate}
  ...
  return (
    <>
      <div className="fixed inset-0 flex overflow-hidden bg-slate-50">
            ...
            <div className="no-scrollbar flex min-h-0 flex-1 flex-col justify-between overflow-y-auto p-3">
              <nav>
                <ul className="grid gap-1.5">
                  {renderNavItems({ onNavigate: () => setIsMobileOpen(false) })}
                </ul>
              </nav>
        ...
        <div
          className="group/sidebar peer hidden text-slate-700 md:block"
            ...
            <aside className="flex h-full w-full flex-col border-r border-slate-200 bg-white">
              ...
              <div className="no-scrollbar my-2 flex min-h-0 flex-1 flex-col justify-between overflow-y-auto group-data-[collapsible=icon]/sidebar:overflow-visible">
                ...
                <div className="flex flex-col gap-2 p-2">
                  {renderLogout({ collapsed: isCollapsed })}
                </div>
              </div>
            </aside>
          </div>
        ...
        <div className="flex h-full w-0 min-w-0 flex-grow flex-col">
          <header className="z-30 flex h-14 shrink-0 items-center justify-between border-b border-slate-200 bg-white px-3 sm:px-4 md:h-12">
            <div className="flex min-w-0 items-center gap-3">
              <button
                ...
                className="inline-flex size-9 items-center justify-center rounded-md text-slate-600 transition hover:bg-slate-100 hover:text-slate-950 md:hidden"
              >
              ...
              <button
                ...
                className="hidden size-9 items-center justify-center rounded-md text-slate-600 transition hover:bg-slate-100 hover:text-slate-950 md:inline-flex"
              >
          ...
          <div className="admin-scrollbar min-h-0 flex-grow overflow-auto bg-slate-50">
            {children}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="flex h-12 shrink-0 items-center border-b border-slate-200 p-2">
      <Link
        ...
        onClick={onNavigate}
        className="flex h-8 w-full min-w-0 items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm font-medium transition-[width,height,padding] hover:bg-slate-100 group-data-[collapsible=icon]/sidebar:!size-8 group-data-[collapsible=icon]/sidebar:!p-2"
      >
        <span className="flex size-4 shrink-0 items-center justify-center rounded-sm bg-slate-950 text-[0.65rem] font-bold text-white">
          {brand.mark ?? "P"}
        </span>
        <span
          className={cn(
      ...
      {!collapsed && onNavigate ? (
        <button
          type="button"
          onClick={onNavigate}
          className="ml-auto inline-flex size-9 items-center justify-center rounded-md text-slate-500 transition hover:bg-slate-100 hover:text-slate-950 md:hidden"
          aria-label="Close menu"
        >
