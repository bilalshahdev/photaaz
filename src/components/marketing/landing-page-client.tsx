...
export function LandingPageClient({
  ...
  const activeHeroSlide = heroSlides[heroImageIndex] ?? heroSlides[0];
  ...
  const isBlankHero = activeHeroSlide?.blank ?? false;
  const slideHeadline = resolveLocalizedString(
    activeHeroSlide?.headline ?? "",
    locale,
  );
  const slideSubheadline = resolveLocalizedString(
    activeHeroSlide?.subheadline ?? "",
    locale,
  );
  const hero = {
    headline: isBlankHero
      ? ""
      : slideHeadline || resolveLocalizedString(settings.hero.headline, locale),
    subheadline: isBlankHero
      ? ""
      : slideSubheadline ||
        resolveLocalizedString(settings.hero.subheadline, locale),
  ...
  const isSectionVisible = (key: keyof typeof settings.sections) =>
    settings.sections[key]?.enabled;
  const sectionOrder = (key: keyof typeof settings.sections) =>
    settings.sections[key]?.displayOrder ?? 99;
  ...
  const subscriptionPlans = pricingPlans.filter(
    (plan) => plan.key !== "ownership",
  );
  ...
  useEffect(() => {
    ...
    }, settings.hero.carouselIntervalSeconds * 1000);

    return () => window.clearInterval(timer);
  }, [heroSlides.length, settings.hero.carouselIntervalSeconds]);
  ...
  function syncThemeCarouselIndex() {
    ...
    themeScrollSyncTimeoutRef.current = window.setTimeout(() => {
      ...
      const viewportCenter =
        currentCarousel.scrollLeft + currentCarousel.clientWidth / 2;
  ...
  return (
    <main
      dir={isRtl ? "rtl" : "ltr"}
      className="flex flex-col bg-[#f7f8f6] text-[#101418]"
    >
      <JsonLd
        data={[
          softwareApplicationJsonLd(appConfig, pricingPlans),
          ...(activeFaqs.length ? [faqJsonLd(activeFaqs)] : []),
        ]}
      />
      <MarketingHeaderStack
      ...
      {isSectionVisible("hero") ? (
        <section
          className="relative h-[100svh] overflow-hidden bg-[#101418] text-white"
          style={{ order: sectionOrder("hero") }}
        >
          {heroSlides.map((image, index) => (
            <div
              key={image.id}
              className={`absolute inset-0 transition-opacity duration-1000 ${index === heroImageIndex ? "z-[1] opacity-100" : "pointer-events-none opacity-0"}`}
            >
              <Image
                src={image.image}
                alt={image.alt}
          ...
          <div className="pointer-events-none absolute inset-0 z-[2] bg-[linear-gradient(90deg,rgba(8,8,7,0.82)_0%,rgba(8,8,7,0.54)_42%,rgba(8,8,7,0.16)_100%)]" />
          {activeHeroSlide?.linkUrl ? (
            <a
              href={activeHeroSlide.linkUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="absolute inset-0 z-[5]"
              aria-label={activeHeroSlide.alt || "Open hero link"}
            />
          ) : null}
          {heroSlides.length > 1 ? (
              ...
              <div className="flex items-center gap-2 rounded-full bg-black/20 px-3 py-2 backdrop-blur-sm">
                {heroSlides.map((image, index) => (
                  <button
                    ...
                    className={`rounded-full transition-all duration-300 ${index === heroImageIndex ? "size-2.5 bg-primary-light" : "size-1.5 bg-white/55 hover:bg-white/80"}`}
          ...
          <MarketingContainer className="pointer-events-none relative z-10 flex h-full items-center pb-14 pt-40 sm:pt-44 lg:pt-48">
            <section
              className={`pointer-events-auto w-full max-w-4xl ${isRtl ? "text-right" : ""}`}
            >
              {hero.headline ? (
                <h1
                  className="max-w-4xl font-light leading-[1.02] tracking-[-0.055em]"
                  style={{
                    color: settings.hero.headlineColor,
                    fontFamily: brandFontVarMap[settings.hero.headlineFont],
                    fontSize: `clamp(2.5rem, 6vw, ${settings.hero.headlineSize}px)`,
                  }}
                >
                  {hero.headline}
                </h1>
              ...
              {!isBlankHero && (activeHeroSlide?.showButtons ?? true) ? (
                <div className="mt-9 flex flex-wrap gap-3">
                  <Button
                    asChild
                    size="lg"
                    className="rounded-none bg-white px-6 font-nav text-xs font-semibold uppercase tracking-[0.22em] text-[#101418] hover:bg-white/90"
                  >
                  ...
                  <Button
                    ...
                    size="lg"
                    variant="secondary"
                    className="rounded-none border border-white/24 bg-black/24 px-6 font-nav text-xs font-semibold uppercase tracking-[0.22em] text-white hover:bg-black/38"
                  >
      ...
      {isSectionVisible("themes") ? (
        <section
          id="themes"
          className="py-20"
          style={{ order: sectionOrder("themes") }}
        >
          <MarketingContainer>
            <div className="border-b border-[#d7dedb] pb-5 lg:pb-8">
              <div className="max-w-4xl">
                <p className="font-nav text-sm font-semibold uppercase tracking-[0.28em] text-primary">
                  {copy.themesEyebrow}
                </p>
            ...
            <MarketingContainer className="flex items-center gap-2 py-3 lg:justify-end">
              <div className="flex shrink-0 gap-2">
                <Button
                  type="button"
                  variant="outline"
                  onClick={() => scrollThemeCarousel("previous")}
                  className="size-11 rounded-none border-[#101418] bg-transparent p-0"
                >
              ...
              <Button
                asChild
                variant="outline"
                className="h-11 min-w-0 flex-1 rounded-none border-[#101418] bg-transparent px-3 font-nav text-[0.68rem] font-semibold uppercase tracking-[0.14em] sm:text-xs sm:tracking-[0.18em] lg:max-w-xs lg:flex-none lg:px-4 lg:tracking-[0.22em]"
              >
          ...
          <MarketingContainer>
            <div className="relative mt-10 overflow-hidden">
              ...
              >
                {themes.map(
                  ({ name, slug, image, description, features, iconKey }) => {
                    const Icon = getThemeIcon(iconKey);
                    const themeName = resolveLocalizedString(name, locale);
                    const themeDescription = resolveLocalizedString(
                      description,
                      locale,
                    );
                    const themeFeatures = features.map((feature) =>
                    ...
                    return (
                      <article
                        key={slug}
                        className="group relative grid min-w-[88vw] snap-start overflow-hidden border border-[#d7dedb]/80 bg-white shadow-[0_18px_55px_rgba(16,20,24,0.08)] transition-shadow duration-300 hover:shadow-[0_28px_80px_rgba(16,20,24,0.14)] sm:min-w-[620px] xl:min-w-[760px] md:grid-cols-[1.12fr_0.88fr]"
                      >
                        ...
                        <div className="flex min-h-[360px] flex-col p-7">
                          ...
                          <h3 className="mt-8 font-display text-5xl font-light leading-none tracking-[-0.055em] break-words">
                            {themeName}
                          </h3>
                          <p className="text-sm leading-6 text-[#59636b]">
                            {themeDescription}
                          </p>
                          ...
                          <div className="mt-auto grid gap-2 pt-7">
                            <Link
                              href={localizePath(locale, themePath(slug))}
                              className="inline-flex h-11 items-center justify-center border border-[#d7dedb] bg-[#ffffff] font-nav text-[0.68rem] font-semibold uppercase tracking-[0.18em] text-[#101418] transition hover:border-[#101418]"
                            >
                              {copy.themeDetails}
                            </Link>
                            <Link
                              href={localizePath(locale, themeDemoPath(slug))}
                              className="inline-flex h-11 items-center justify-center gap-2 bg-[#101418] font-nav text-[0.68rem] font-semibold uppercase tracking-[0.18em] text-white transition hover:bg-primary/90"
                            >
                              {copy.liveDemo}
                              <ExternalLink
                                className="size-3.5"
            ...
            {themes.length > 1 ? (
              <div
                ...
                aria-label="Theme carousel position"
              >
                <div className="relative flex items-center gap-2">
                  <span
                    ...
                    style={{
                      transform: `translate(${activeThemeIndex * 2}rem, -50%)`,
                    }}
                    aria-hidden="true"
                  />
                  {themes.map((theme, index) => (
                    <button
                      ...
                      aria-current={
                        index === activeThemeIndex ? "true" : undefined
                      }
                    >
                      <span
                        className="h-1.5 w-4 rounded-full bg-[#cbd5d1] transition-colors duration-300 hover:bg-primary"
                        aria-hidden="true"
                      />
                    </button>
                  ))}
      ...
      {isSectionVisible("features") ? (
        <section
          ...
          style={{ order: sectionOrder("features") }}
        >
          <div className="mx-auto max-w-7xl">
            <div
              className={`border-b border-[#d7dedb] pb-8 ${isRtl ? "text-right" : ""}`}
            >
              <p className="font-nav text-sm font-semibold uppercase tracking-[0.28em] text-primary">
            ...
            <div className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {settings.features
                ...
                .map(({ title, body }, index) => {
                  const Icon =
                    benefitFeatures[index % benefitFeatures.length]?.icon ??
                    CheckCircle2;
                  ...
                  return (
                    <article
                      ...
                      className="border border-[#d7dedb] bg-[#f7f8f6] p-5"
                    >
                      <Icon
      ...
      {isSectionVisible("pricing") ? (
        <section
          ...
          style={{ order: sectionOrder("pricing") }}
        >
          <div className="mx-auto max-w-7xl">
            <div
              className={`border-b border-[#d7dedb] pb-8 ${isRtl ? "text-right" : ""}`}
            ...
            <div className="mt-8 grid gap-4 lg:grid-cols-3">
              {subscriptionPlans.map((plan) => {
                ...
                const planDescription = resolveLocalizedString(
                  plan.description,
                  locale,
                );
                ...
                return (
                  <article
                    ...
                    className={
                      plan.featured
                        ? "border-2 border-[#101418] bg-[#101418] p-6 text-white"
                        : "border border-[#d7dedb] bg-white p-6"
                    }
                  >
                    ...
                    <p
                      className={
                        plan.featured
                          ...
                          : "mt-4 text-sm leading-6 text-[#59636b]"
                      }
                    >
                      {planDescription}
                    </p>
      ...
      {isSectionVisible("faq") && activeFaqs.length > 0 ? (
        <section
          id="faq"
          className="bg-[#f7f8f6] px-4 py-20 sm:px-6 lg:px-8"
            ...
            >
                ...
                <div
                  className={
                    contactStatus === "success"
                      ? "mb-5 border border-primary/20 bg-primary/5 p-4 text-sm text-primary"
                      : "mb-5 border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"
                  }
                >
                  {contactStatus === "success"
                    ? copy.contactSuccess
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <Label className="block">
                  <span className="font-nav text-xs font-semibold uppercase tracking-[0.22em] text-primary">
                    {copy.nameLabel}
                  </span>
              ...
              <Label className="mt-4 block">
                ...
                <span className="mt-2 flex min-h-40 items-start border border-[#d7dedb] bg-[#ffffff] px-4 py-4">
                  <MessageSquareText
                    className="mr-3 mt-1 size-5 shrink-0 text-primary"
                    aria-hidden="true"
                  />
                  <Textarea
                    required
              ...
              <Button
                type="submit"
                className="mt-5 h-11 rounded-none bg-[#101418] px-6 font-nav text-xs font-semibold uppercase tracking-[0.22em] text-white hover:bg-primary/90"
              >
                {contact.submitLabel}
              </Button>
            </form>
      ...
      {isSectionVisible("finalCta") ? (
        <section
          className="bg-[#101418] px-4 py-14 text-white sm:px-6 lg:px-8"
          style={{ order: sectionOrder("finalCta") }}
        >
          <div className="mx-auto flex max-w-7xl flex-col items-start justify-between gap-8 md:flex-row md:items-end">
            <div>
              <p className="font-nav text-sm font-semibold uppercase tracking-[0.28em] text-primary-light">
                {copy.launchEyebrow}
              </p>
              <h2 className="pf-fluid-title-page mt-3 max-w-4xl font-display font-light leading-none tracking-[-0.05em]">
                {copy.launchTitle}
...
function MarketingFooter({
  ...
  const showSalesEmail = Boolean(
    config.salesEmail && config.salesEmail !== config.supportEmail,
  );
  const showPhone = config.phone.enabled && Boolean(config.phone.value);
  const showCreator =
    config.creatorLink.enabled && isHttpUrl(config.creatorLink.href);
  const creatorLabel = config.creatorLink.label
    .replace(/^(built|made)\s+by\s+/i, "")
    .trim();
  const copyrightText = resolveLocalizedString(
  ...
  return (
    <footer
      className="border-t border-[#20272b] bg-[#101418] text-white"
      style={{ order: 1000 }}
    >
        ...
        <div className="mx-auto max-w-7xl">
          <div className="grid gap-8 lg:grid-cols-[minmax(0,1.25fr)_minmax(180px,0.45fr)_minmax(230px,0.65fr)] lg:items-start">
            <div className="max-w-xl">
              <Link
                ...
                className="font-brand text-5xl font-semibold tracking-[-0.05em] text-white"
              >
                Photaaz
              </Link>
              {config.footerText ? (
                <p className="mt-3 max-w-xl truncate text-xs leading-6 text-white/62">
                  {resolveLocalizedString(config.footerText, locale)}
                </p>
              ) : null}
              <div className="mt-5 flex flex-wrap gap-3 text-sm text-white/75">
                {showSupportEmail ? (
                  <a
                    ...
                    className="inline-flex items-center gap-2 border border-white/10 bg-white/[0.03] px-3 py-2 transition hover:border-primary-light/60 hover:text-primary-light"
                  >
                ...
                {config.companyAddress ? (
                  <span className="inline-flex items-center border border-white/10 bg-white/[0.03] px-3 py-2">
                    {config.companyAddress}
                  </span>
                ) : null}
              </div>
            ...
            <div>
              <p className="font-nav text-xs font-semibold uppercase tracking-[0.24em] text-primary-light">
                {translatePlatformLiteral("Legal", locale)}
              </p>
              <nav className="mt-4 grid gap-2.5 text-sm text-white/72">
                {legalLinks.map((link) => (
                  <Link
                    key={link.slug}
                    href={localizePath(locale, legalPath(link.slug))}
                    className="w-fit transition hover:translate-x-1 hover:text-primary-light"
                  >
                    {translatePlatformLiteral(link.label.en, locale)}
                  </Link>
                ))}
              </nav>
            </div>
            ...
            <div className="space-y-4">
              <p className="font-nav text-xs font-semibold uppercase tracking-[0.24em] text-primary-light">
                {translatePlatformLiteral("Social", locale)}
              </p>
              {socialLinks.length ? (
              ...
              <div className="flex flex-col gap-3 text-sm text-white/72">
                {showPhone ? (
                  <a
                    ...
                    className="inline-flex items-center gap-2 transition hover:text-primary-light"
                  >
              ...
              {showSalesEmail ? (
                <a
                  ...
                  className="inline-flex items-center gap-2 text-sm text-white/72 transition hover:text-primary-light"
                >
                  <Mail className="size-4" aria-hidden="true" />
          ...
          <div className="mt-9 grid gap-4 border-t border-white/10 pt-5 text-center text-xs font-medium uppercase tracking-[0.16em] text-white/48 sm:grid-cols-3 sm:items-center">
            ...
            {showCreator && creatorLabel ? (
              <a
                href={config.creatorLink.href}
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center justify-center gap-2 text-primary-light transition hover:text-white sm:justify-self-end"
              >
                {translatePlatformLiteral("Made by", locale)} {creatorLabel}
...
function OwnershipPricingCard({
  plan,
  locale,
}: {
  plan: PlatformPricingPlanView;
  locale: AppLocale;
}) {
  const planName = resolveLocalizedString(plan.name, locale);
  ...
  return (
    <article className="border border-primary/30 bg-[#f3f7f3] p-6 text-[#101418] shadow-[0_24px_70px_rgba(16,20,24,0.10)] lg:col-span-3">
      <div className="grid gap-8 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)] lg:items-center">
        <div>
          <p className="font-nav text-xs font-semibold uppercase tracking-[0.24em] text-primary">
            {translatePlatformLiteral("Permanent ownership", locale)}
          </p>
          <h3 className="mt-4 font-display text-5xl font-light leading-none tracking-[-0.06em]">
            {planName}
          </h3>
          <MarketingPrice
            monthlyPrice={plan.monthlyPrice}
            ...
            locale={locale}
          />
          <p className="mt-5 max-w-xl text-sm leading-7 text-[#59636b]">
            {planDescription}
          </p>
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {planFeatures.slice(0, 8).map((feature) => (
            <div
              key={feature}
              className="flex gap-3 border border-primary/15 bg-white/75 p-4 text-sm text-[#334155]"
            >
              <CheckCircle2
                className="mt-0.5 size-4 shrink-0 text-primary"
                aria-hidden="true"
...
function MarketingPrice({
  ...
  return (
    <div className="mt-6 flex items-end gap-2">
      <span className="font-display text-6xl font-light leading-none tracking-[-0.06em]">
        {price.amount}
      </span>
      <span
        className={
          featured
            ? "pb-2 text-sm text-white/58"
            : "pb-2 text-sm text-[#59636b]"
        }
      >
        {price.suffix}
      </span>
...
function getMarketingPrice(
  ...
  if (plan.monthlyPrice && plan.monthlyPrice > 0) {
    return {
      amount: formatMarketingAmount(plan.monthlyPrice),
      suffix: `/ ${translatePlatformLiteral("month", locale)}`,
    };
  }

  if (plan.annualPrice && plan.annualPrice > 0) {
    return {
      amount: formatMarketingAmount(plan.annualPrice),
      suffix: `/ ${translatePlatformLiteral("year", locale)}`,
    };
  }

  if (plan.lifetimePrice && plan.lifetimePrice > 0) {
    return {
      amount: formatMarketingAmount(plan.lifetimePrice),
      suffix: translatePlatformLiteral("one time", locale),
    };
  }
...
function formatMarketingAmount(value: number) {
  ...
  return (
    <article className="border border-[#d7dedb] bg-white p-5 shadow-[0_18px_55px_rgba(16,20,24,0.06)]">
      ...
      >
        <span>{question}</span>
        <ChevronDown
          className={`mt-1 size-5 shrink-0 text-primary transition duration-300 ${isOpen ? "rotate-180" : ""}`}
          aria-hidden="true"
        />
      </button>
      <div
