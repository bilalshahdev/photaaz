...
import { assertCategoryRequestLimit, assertGalleryPhotoLimit, assertPhotoUploadLimits } from "@/services/subscription/plan-limits";
...
export async function createCustomerPhoto(formData: FormData) {
  ...
  const upload = await saveTenantImageUpload(imageFile, parsed.tenantSlug, { area: "photos", fileLabel: parsed.title });
