...
export default async function CustomerCategoriesPage({ params }: CustomerCategoriesPageProps) {
  ...
  const categories = site.categories?.length ? site.categories : isDemo ? customerGalleryTaxonomy[variant] : [];
  ...
  return (
    <CustomerPublicPage
      ...
      pageKey="categories"
    >
      <CustomerCategoryBrowser slug={slug} galleries={site.galleries} categories={categories} variant={variant} imageWatermark={site.imageWatermark} isDemo={isDemo} />
    </CustomerPublicPage>
  );
}
